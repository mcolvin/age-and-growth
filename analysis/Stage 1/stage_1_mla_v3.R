...
for(kk in c(1:nrow(fish)))
	...
	for(i in c(1:length(ss)))
		...
		for(x in c(1:nsims))
			...
			srs<- dcast(pop[srs_indx,],
				age~"mla_srs",
				value.var="len_obs",
				mean)
			xx<- merge(mla,srs,by="age",all.x=TRUE)
			out<- data.frame(
				species=kk,
				spp=spp,
				A=A,
				...
				sim=x,
				age=xx$age,
				mla_age_true=xx$mla_pop,
				mla_age_srs=xx$mla_srs)
		if(kk==1 & i==1 & x==1)
				{write.csv(out,"./output/stage_1_mla.csv")} else
				{write.table(out,"./output/stage_1_mla.csv",
