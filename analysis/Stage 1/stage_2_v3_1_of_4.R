...
for(kk in c(1:nrow(stage1)))
	...
	A<- 1-exp(-a*maxAge^b )
	ss_ma<- stage1$ma[kk]
	ss_mla<- stage1$mla[kk]
	ss_af<- stage1$af[kk]
	mape<-stage1$level[kk]
	...
	for(n in c(1:2))
		...
		bin_func<- approxfun(bins,bins, method="constant",rule=2)
		...
		for(ll in c(1:length(bin_n)))
			...
			for(i in c(1:nsims))
				...
				if("ma_stage_2.csv" %in% dir("./output")==FALSE)
					{write.csv(stage_2_ma,"./output/ma_stage_2.csv")} else
					{write.table(stage_2_ma,"./output/ma_stage_2.csv",append=TRUE, col.names=FALSE,sep=',')}		
				...
				if("af_stage_2.csv" %in% dir("./output")==FALSE)
					{write.csv(stage_2_af,"./output/af_stage_2.csv")} else
					{write.table(stage_2_af,"./output/af_stage_2.csv",append=TRUE, col.names=FALSE,sep=',')}
				...
				if("mla_stage_2.csv" %in% dir("./output")==FALSE)
					{write.csv(stage_2_mla,"./output/mla_stage_2.csv")} else
					{write.table(stage_2_mla,"./output/mla_stage_2.csv",append=TRUE, col.names=FALSE,sep=',')}						
				}# i
			}# ll
		} #n
	} # kk
